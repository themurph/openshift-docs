...
When planning a build of a RHEL based docker-formatted container that includes a package that will require a subscription (or "entitlement") that is not enabled on the build host, a deeper understanding of how subscriptions are inherited from the build host is needed to ensure a successful build.
